...
function nbhood_display(point) {
    ...
    if (nbhood.length < 1) { return }
    document.querySelector("#nbhd_name").innerHTML = `
        <h3 style="margin:7px 0 1px 0">${nbhood[0].properties.nbhd_name}</h3>`

    document.querySelector(".content").innerHTML = `
        <p><b>Price Avg:</b> ${formatter.format(nbhood[0].properties.total_valu_avg)}</p>
        <p><b>Land Value Avg:</b> ${formatter.format(nbhood[0].properties.land_value_avg)}</p>
        <p><b>Improvement Avg:</b> ${formatter.format(nbhood[0].properties.improvemen_avg)}</p>
        <p><b>Total Value Min:</b> ${formatter.format(nbhood[0].properties.total_valu_min)}</p>
        <p><b>Total Value Max:</b> ${formatter.format(nbhood[0].properties.total_valu_max)}</p>
        <p><b># of Homes:</b> ${nbhood[0].properties.num_of_houses}</p>
        <p><b>Avg Lot Size:</b> ${nbhood[0].properties.average_lot_acreage} acre</p>
    `
}
...
map.on('load', function () {
    ...
    map.on('click', 'parcels_fill', function (e) {
        ...
        let description = `
            ...
            <tr><td style="font-size:15px;">Total Value</td>
                <td id="studentid" style="font-weight:500;font-size:15px;">${formatter.format(p.total_valu_og)}</td>
            </tr>
            ...
            <a style="text-align:center;" target="_blank" href="https://www.denvergov.org/property/realproperty/summary/${p.pin}">More Info</a>
            `;
        new mapboxgl.Popup()
            .setLngLat(e.lngLat)
            .setHTML(description)
    ...
    //Neighborhoods
    map.on('click', 'neighborhoods_fill', function (e) { //console.log(e.features)
        if (map.getZoom() > 15) { return }
        let p = e.features[0].properties;
        let description = `
        <table class="table_color" style="margin-top:10px;">
            <h4 style="text-align:center; margin:0 0 0 0;">${p.nbhd_name}</h4>
            <tr><td>Price Avg</td><td> ${formatter.format(p.total_valu_avg)} </td></tr>
            <tr><td>Land Value</td><td> ${formatter.format(p.land_value_avg)} </td></tr>
            <tr><td>Improvement Avg</td><td> ${formatter.format(p.improvemen_avg)} </td></tr>
            <tr><td>Total Value Min</td><td>  ${formatter.format(p.total_valu_min)} </td></tr>
            <tr><td>Total Value Max</td><td> ${formatter.format(p.total_valu_max)} </td></tr>
